...
document.getElementById('satellite-info').textContent = `📍方位：° 仰角：° | 🔝最高：${selectedPass.highestElevation}° `;
...
document.getElementById('info-btn').textContent = "多普勒";
...
const noradId = satelliteTLE2 ? satelliteTLE2.split(' ')[1] : '';
document.getElementById('satellite-name').textContent = `🛰${selectedPass.satelliteName} [${noradId}]`;
            document.getElementById('satellite-info').textContent = `📍方位：${azimuthdg}° 仰角：${elevationdg}° | 🔝最高：${selectedPass.highestElevation}° `;
            document.getElementById('pass-info').textContent = `🔼开始：${formattedEntryTime}  | 🔽结束：${formattedExitTime}`;
...
function fetchSatelliteData(noradCatId, selectedsat) {
  ...
  fetch('/satdata/transmitters.json')
    ...
    .then(allData => {
      ...
      // 修改这里：无论是否找到数据，都调用 displayFrequencyData
      if (filteredData.length === 0) {
        // 没有找到数据时，传递空数组
        const freqInfo = JSON.parse(localStorage.getItem('freqinfo') || '{}');
        freqInfo[noradCatId] = []; // 存储空数组
...
function updateDoppler(downlink_low, uplink_low,description) {
     ...
     // 检查是否是手动输入（description为'manual'）
    const isManual = description === 'manual';
    
    // 如果上行频率是'null'字符串，转换为null
    if (uplink_low === 'null') {
        uplink_low = null;
    }
    ...
    content += `
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin: 10px 0;">
            ...
            <button style="margin-left: 10px; padding: 5px 10px; background-color: #2e71d1; color: white; border: none; border-radius: 5px; cursor: pointer;" onclick="startManualDoppler()">
                ${currentLang == 'zh' ? '多普勒' : 'Doppler Display'}
            </button>
        </div>
